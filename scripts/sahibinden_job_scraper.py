...
class SahibindenJobScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sahibinden İş İlanı Çekme ve Paylaşma Uygulaması")
        ...
        ttk.Label(header_frame, text="Sahibinden İş İlanı Çekme ve Paylaşma Uygulaması", style='Header.TLabel').pack(side=tk.LEFT)
        ...
        # Toplu çekme butonu
        self.bulk_scrape_button = ttk.Button(button_frame, text="Toplu İlan Çek", command=self.show_bulk_scrape_dialog)
        self.bulk_scrape_button.pack(side=tk.LEFT, padx=(0, 5))
        ...
        # İletişim telefonu
        ttk.Label(form_content, text="İletişim Telefonu:").grid(row=8, column=0, sticky=tk.W, pady=5)
        self.phone_entry = ttk.Entry(form_content, width=50, font=('Segoe UI', 10))
        self.phone_entry.grid(row=8, column=1, sticky=tk.EW, pady=5, padx=(5, 0))
    ...
    def setup_form_tracking(self):
        ...
        self.phone_entry.bind("<KeyRelease>", self.update_preview)
    ...
    def update_preview(self, event=None):
        ...
        try:
            ...
            preview_text += f"İletişim E-posta: {email}\n"
            if phone:
                preview_text += f"İletişim Telefonu: {phone}\n\n"
            else:
                preview_text += "\n"
    ...
    def extract_phone_number(self, text):
        ...
        # Telefon numarası formatını ara
        phone_pattern = r'0\s*\(\s*\d{3}\s*\)\s*\d{3}\s*\d{2}\s*\d{2}'
        match = re.search(phone_pattern, text)
        
        if match:
            # Boşlukları ve parantezleri kaldır
            phone = match.group(0)
    ...
    def _scrape_job_thread(self, url):
        ...
        try:
            ...
            # Telefon numarası
            phone_element = soup.select_one('.pretty-phone-part')
            phone = ""
            if phone_element:
                phone_text = phone_element.text
                phone = self.extract_phone_number(phone_text)
                self.log(f"Telefon: {phone}")
            ...
            if phone:
                self.root.after(0, lambda: self.phone_entry.insert(0, phone))
    ...
    def determine_category(self, job_area, position):
        ...
        category_mapping = {
            ...
            "Restoran": ["hizmet", "garson"],
            "Konaklama": ["turizm", "resepsiyonist"],
            "Kebap": ["hizmet", "asci"],
            "Döner": ["hizmet", "asci"]
        }
    ...
    def validate_job_data(self, job_data):
        ...
        if not job_data['title']:
            messagebox.showerror("Hata", "Lütfen bir başlık def validate_job_data(self, job_data):
        """İş ilanı verisini doğrular"""
        if not job_data['title']:
        ...
        if not job_data['contactEmail'] and not job_data['contactPhone']:
            messagebox.showerror("Hata", "Lütfen en az bir iletişim bilgisi (e-posta veya telefon) girin")
            return False
    ...
    def load_from_json(self):
        ...
        try:
            ...
            # İletişim telefonu
            self.phone_entry.delete(0, tk.END)
            self.phone_entry.insert(0, job_data.get('contactPhone', ''))
    ...
    def show_bulk_scrape_dialog(self):
        """Toplu çekme diyaloğunu gösterir"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Toplu İlan Çekme")
        ...
        # Kategori URL'si
        ttk.Label(frame, text="Kategori URL'si:").grid(row=0, column=0, sticky=tk.W, pady=5)
        url_entry = ttk.Entry(frame, width=50)
        url_entry.grid(row=0, column=1, sticky=tk.EW, pady=5)
        url_entry.insert(0, "https://www.sahibinden.com/is-ilanlari")
        
        # Sayfa sayısı
        ttk.Label(frame, text="Sayfa Sayısı:").grid(row=1, column=0, sticky=tk.W, pady=5)
        page_spinbox = ttk.Spinbox(frame, from_=1, to=10, width=10)
        page_spinbox.grid(row=1, column=1, sticky=tk.W, pady=5)
        page_spinbox.set(2)
        
        # İlan sayısı
        ttk.Label(frame, text="Sayfa Başına İlan Sayısı:").grid(row=2, column=0, sticky=tk.W, pady=5)
        job_spinbox = ttk.Spinbox(frame, from_=1, to=20, width=10)
        job_spinbox.grid(row=2, column=1, sticky=tk.W, pady=5)
        job_spinbox.set(3)
        
        # İlerleme çubuğu
        progress_var = tk.DoubleVar()
        progress = ttk.Progressbar(frame, variable=progress_var, maximum=100)
        progress.grid(row=3, column=0, columnspan=2, sticky=tk.EW, pady=10)
        ...
        log_text.grid(row=5, column=0, columnspan=2, sticky=tk.NSEW, pady=5)
        log_text.config(state=tk.DISABLED)
        
        # Butonlar
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=6, column=0, columnspan=2, sticky=tk.EW, pady=10)
        ...
        def start_bulk_scraping():
            category_url = url_entry.get().strip()
            max_pages = int(page_spinbox.get())
            max_jobs_per_page = int(job_spinbox.get())
            
            if not category_url:
                messagebox.showerror("Hata", "Lütfen bir kategori URL'si girin", parent=dialog)
                return
            ...
            # Arka planda çalıştır
            threading.Thread(
                target=self._bulk_scrape_thread,
                args=(category_url, max_pages, max_jobs_per_page, log_message, update_status, update_progress, dialog, start_button, cancel_button),
                daemon=True
            ).start()
        
        start_button = ttk.Button(button_frame, text="Başlat", command=start_bulk_scraping)
    ...
    def _bulk_scrape_thread(self, category_url, max_pages, max_jobs_per_page, log_func, status_func, progress_func, dialog, start_button, cancel_button):
        """Arka planda toplu iş ilanı çekme işlemi"""
        try:
            status_func("İlan URL'leri çekiliyor...")
            log_func(f"Kategori URL'si: {category_url}")
            log_func(f"Sayfa sayısı: {max_pages}")
            log_func(f"Sayfa başına ilan sayısı: {max_jobs_per_page}")
            
            # İlan URL'lerini al
            job_urls = self.get_job_listings_from_sahibinden(category_url, max_pages, max_jobs_per_page, log_func, progress_func)
            
            if not job_urls:
                log_func("Hiç ilan URL'si bulunamadı.")
                status_func("Tamamlandı - Hiç ilan bulunamadı")
                dialog.after(0, lambda: start_button.config(state=tk.NORMAL))
                dialog.after(0, lambda: cancel_button.config(state=tk.NORMAL))
                return
            
            total_jobs = len(job_urls)
            log_func(f"Toplam {total_jobs} ilan URL'si bulundu.")
            ...
            for i, url in enumerate(job_urls):
                try:
                    current_progress = (i / total_jobs) * 100
                    progress_func(current_progress)
                    status_func(f"İlan çekiliyor ({i+1}/{total_jobs}): {url}")
                    log_func(f"İlan çekiliyor: {url}")
                    
                    # İlanı çek
                    job_data = self.scrape_job_from_sahibinden(url, log_func)
                    
                    if job_data:
                        # Firebase'e kaydet
                        job_id = self.save_to_firebase(job_data, log_func)
                        
                        if job_id:
                            log_func(f"İlan başarıyla eklendi. İlan ID: {job_id}")
                            successful_jobs += 1
                        else:
                            log_func("İlan eklenemedi veya zaten mevcut.")
                    else:
                        log_func("İlan çekilemedi.")
                    
                    # İlanlar arası bekleme
                    time.sleep(random.uniform(1, 3))
                    
                except Exception as e:
                    log_func(f"İlan işlenirken hata: {e}")
                    continue
            
            progress_func(100)
            status_func(f"Tamamlandı - {successful_jobs}/{total_jobs} ilan eklendi")
            log_func(f"İşlem tamamlandı. {successful_jobs}/{total_jobs} ilan başarıyla eklendi.")
            
        except Exception as e:
            log_func(f"Toplu çekme işlemi sırasında hata: {e}")
            status_func("Hata")
    ...
    def get_job_listings_from_sahibinden(self, category_url, max_pages=3, max_jobs_per_page=5, log_func=None, progress_func=None):
        """
        ...
        for page in range(1, max_pages + 1):
            try:
                ...
                log_func(f"Sayfa çekiliyor: {page_url}")
                
                response = requests.get(page_url, headers=headers)
                
                if response.status_code != 200:
                    log_func(f"Hata: Sayfa yüklenemedi. Durum kodu: {response.status_code}")
                    continue
                ...
                for card in job_cards[:max_jobs_per_page]:
                    try:
                        ...
                        log_func(f"İlan URL'si bulundu: {detail_url}")
                    
                    except Exception as e:
                        log_func(f"İlan kartı işlenirken hata: {e}")
                        continue
                
                log_func(f"Sayfa {page} tamamlandı.")
                
                if progress_func:
                    progress_value = (page / max_pages) * 50  # İlk %50'lik kısım URL toplama
                ...
                log_func(f"Sayfa {page} çekilirken hata: {e}")
                continue
        
        return job_urls

    def scrape_job_from_sahibinden(self, url, log_func=None):
        """
        ...
        try:
            ...
            log_func("Sayfa yüklendi, içerik ayrıştırılıyor...")
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # İlan başlığı
            title_element = soup.select_one('.classifiedDetailTitle h1')
            title = self.clean_text(title_element.text) if title_element else "İş İlanı"
            ...
            # Kategori ve alt kategori eşleştirme
            category, sub_category = self.determine_category(job_area, position)
            ...
            log_func(f"İlan başarıyla çekildi: {title}")
            return job_data
        
        except Exception as e:
            log_func(f"İlan çekilirken hata oluştu: {e}")
            return None

    def save_to_firebase(self, job_data, log_func=None):
        """İş ilanını Firebase'e kaydeder"""
        if log_func is None:
            log_func = self.log
        ...
        try:
            ...
            if not query:
                # Yeni ilan ekle
                new_job_ref = jobs_ref.push(job_data)
                log_func(f"İlan başarıyla kaydedildi: {job_data['title']}")
                return new_job_ref.key
            else:
                log_func(f"Bu ilan zaten mevcut: {job_data['title']}")
                return None
            
        except Exception as e:
            log_func(f"İlan kaydedilirken hata: {e}")
            return None

def main():
    root = tk.Tk()
    app = SahibindenJobScraperApp(root)
    root.mainloop()
